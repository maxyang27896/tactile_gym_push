...
def plot_train_results(saved_model_dir, window_size=50, show_plot=False):
    ...
    if timesteps.shape[0] < window_size:
        print(
            "Cannot apply windows of size {} to data of size {}".format(
                window_size, timesteps.shape[0]
            )
        )
        return None, None

    x_r, mean_rew = results_plotter.window_func(timesteps, reward, window_size, np.mean)
    x_l, mean_ep_len = results_plotter.window_func(
        timesteps, ep_lengths, window_size, np.mean
    )
    ...
    _, max_ep_len = results_plotter.window_func(
        timesteps, ep_lengths, window_size, np.max
    )
    _, min_ep_len = results_plotter.window_func(
        timesteps, ep_lengths, window_size, np.min
    )
...
def load_eval_results(saved_model_dir):
    eval_data = np.load(
        os.path.join(saved_model_dir, "trained_models", "evaluations.npz")
    )
...
def plot_train_and_eval(
    saved_model_dir, fig=None, axs=None, show_plot=False, save_plot=False
):
    ...
    train_rew_data, train_len_data = plot_train_results(
        saved_model_dir, show_plot=False
    )
...
def plot_obs_stack(obs_stack, n_obs_channel=1):
    ...
    for i in range(n_stack):
        ...
        n_fig_columns = 4
        n_fig_rows = (
            num_envs // n_fig_columns
            if num_envs % n_fig_columns == 0
            else (num_envs // n_fig_columns) + 1
        )
