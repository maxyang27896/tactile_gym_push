...
class TacTip:
    ...
    def turn_off_tactip_collisions(self):
        ...
        self._pb.setCollisionFilterGroupMask(
            self.robot_id, self.tactip_link_ids['body'], 0, 0
        )

        if self.tactip_type == "right_angle":
            self._pb.setCollisionFilterGroupMask(
                self.robot_id, self.tactip_link_ids['adapter'], 0, 0
            )

        if self.tactip_core == "no_core":
            self._pb.setCollisionFilterGroupMask(
                self.robot_id, self.tactip_link_ids['tip'], 0, 0
            )
    ...
    def save_reference_images(self):
        ...
        border_mask = (mask_base_id == self.robot_id) & (
            mask_link_id == self.tactip_link_ids['body']
        ).astype(np.uint8)
    ...
    def setup_camera_info(self):
        ...
        if self.tactip_type in ["standard", "flat", "right_angle"]:
            ...
            self.fov = 60
        elif self.tactip_type in ["smg"]:
            self.focal_dist = 0.05
            self.fov = 70
    ...
    def update_cam_frame(self):
        ...
        if self.tactip_type in ["standard", "flat"]:
            ...
            cam_pos = (0, 0.0185, 0.0085)
            cam_rpy = (0, 12.5*np.pi/180, -np.pi/2)
    ...
    def camvec_to_worldvec(self, camframe_vec):
        ...
        rot_matrix = np.array(
            self._pb.getMatrixFromQuaternion(self.camframe_orn)
        ).reshape(3, 3)
    ...
    def tactip_camera(self):
        ...
        pen_img = (
            (np.clip(pen_img, 0, max_penetration) / max_penetration) * 255
        ).astype(np.uint8)
    ...
    def reset_tip(self):
        ...
        if self.tactip_core == "no_core":
            ...
            # if dynamics parameters not empty change dynamics
            if self.tactip_dynamics != {}:

                if (self.tactip_dynamics['damping'] is not None and
                    self.tactip_dynamics['stiffness'] is not None):
                ...
                if self.tactip_dynamics['friction'] is not None:
                    self._pb.changeDynamics(
                        self.robot_id,
                        self.tactip_link_ids['tip'],
                        lateralFriction=self.tactip_dynamics['friction']
                    )
