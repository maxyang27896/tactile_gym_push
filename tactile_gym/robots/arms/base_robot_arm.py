...
class BaseRobotArm:
    ...
    def reset(self):
        ...
        for i in range(self.num_joints):
            ...
            self._pb.changeDynamics(
                self.robot_id, i, linearDamping=0.04, angularDamping=0.04
            )
    ...
    def workframe_to_worldframe(self, pos, rpy):
        ...
        worldframe_pos, worldframe_orn = self._pb.multiplyTransforms(
            self.workframe_pos, self.workframe_orn, pos, orn
        )
    ...
    def worldframe_to_workframe(self, pos, rpy):
        ...
        inv_workframe_pos, inv_workframe_orn = self._pb.invertTransform(
            self.workframe_pos, self.workframe_orn
        )
        workframe_pos, workframe_orn = self._pb.multiplyTransforms(
            inv_workframe_pos, inv_workframe_orn, pos, orn
        )
    ...
    def workvec_to_worldvec(self, workframe_vec):
        ...
        rot_matrix = np.array(
            self._pb.getMatrixFromQuaternion(self.workframe_orn)
        ).reshape(3, 3)
    ...
    def worldvec_to_workvec(self, worldframe_vec):
        ...
        rot_matrix = np.array(
            self._pb.getMatrixFromQuaternion(inv_workframe_orn)
        ).reshape(3, 3)
    ...
    def get_current_joint_pos_vel(self):
        ...
        """
        cur_joint_states = self._pb.getJointStates(
            self.robot_id, self.control_joint_ids
        )
    ...
    def get_current_TCP_pos_vel_workframe(self):
        ...
        tcp_pos_workframe, tcp_rpy_workframe = self.worldframe_to_workframe(
            tcp_pos, tcp_rpy
        )
        tcp_orn_workframe = self._pb.getQuaternionFromEuler(tcp_rpy_workframe)
        tcp_lin_vel_workframe, tcp_ang_vel_workframe = self.worldvel_to_workvel(
            tcp_lin_vel, tcp_ang_vel
        )
    ...
    def check_TCP_vel_lims(self, vels):
        ...
        exceed_pos_llims = np.logical_and(
            cur_tcp_pos < self.TCP_lims[:3, 0], vels[:3] < 0
        )
        exceed_pos_ulims = np.logical_and(
            cur_tcp_pos > self.TCP_lims[:3, 1], vels[:3] > 0
        )
        exceed_rpy_llims = np.logical_and(
            cur_tcp_rpy < self.TCP_lims[3:, 0], vels[3:] < 0
        )
        exceed_rpy_ulims = np.logical_and(
            cur_tcp_rpy > self.TCP_lims[3:, 1], vels[3:] > 0
        )
    ...
    def draw_TCP_box(self):
        # self.TCP_lims[0, 0], self.TCP_lims[0, 1] = -0.2, +0.1  # x lims
        # self.TCP_lims[1, 0], self.TCP_lims[1, 1] = -0.1, +0.1  # y lims
        # self.TCP_lims[2, 0], self.TCP_lims[2, 1] = -0.1, +0.1  # z lims

        p1 = [
            self.workframe_pos[0] + self.TCP_lims[1, 0],
            self.workframe_pos[1] + self.TCP_lims[0, 1],
            self.workframe_pos[2],
        ]
        p2 = [
            self.workframe_pos[0] + self.TCP_lims[1, 0],
            self.workframe_pos[1] + self.TCP_lims[0, 0],
            self.workframe_pos[2],
        ]
        p3 = [
            self.workframe_pos[0] + self.TCP_lims[1, 1],
            self.workframe_pos[1] + self.TCP_lims[0, 0],
            self.workframe_pos[2],
        ]
        p4 = [
    ...
    def test_workframe_transforms(self):
        ...
        worldframe_pos, worldframe_rpy = self.workframe_to_worldframe(
        ...
        print(
            "Workframe Position:  {}, Workframe RPY:  {}".format(
                workframe_pos, workframe_rpy
            )
        )
        print(
            "Worldframe Position: {}, Worldframe RPY: {}".format(
                worldframe_pos, worldframe_rpy
            )
        )
        print(
    ...
    def test_workvel_transforms(self):
        ...
        work_lin_vel, work_ang_vel = self.worldvel_to_workvel(
            init_lin_vel, init_ang_vel
        )
        world_lin_vel, world_ang_vel = self.workvel_to_worldvel(
            work_lin_vel, work_ang_vel
        )
        ...
        print(
            "World Lin Vel: {}, World Ang Vel: {}".format(world_lin_vel, world_ang_vel)
        )
        print(
