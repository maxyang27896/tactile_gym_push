...
def demo_rl_env(
    env, num_iter, action_ids, show_gui, show_tactile, render, print_info=False
):
    """
    ...
    if record:
        import imageio
        render_frames = []
        log_id = env._pb.startStateLogging(loggingType=env._pb.STATE_LOGGING_VIDEO_MP4, fileName=os.path.join('example_videos', 'gui.mp4'))

    # collection loop
    for i in range(num_iter):
        r_sum = 0
        o, r, d, ep_ret, ep_len = env.reset(), 0, False, 0, 0
        # env.reset_object(init_orn=np.deg2rad(20), init_pos_y = 0.02)
        step = 0
        ...
        print("Steps: {}, Total Reward: {}".format(step,r_sum))

    if record:
        env._pb.stopStateLogging(log_id)
        imageio.mimwrite(os.path.join('example_videos', 'render.mp4'),
